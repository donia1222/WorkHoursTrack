  ...
  static async generateWorkDays(
    ...
    while (currentDate <= endDate) {
      ...
      if (workDays.includes(dayOfWeek)) {
        ...
        if (!hasAnyWork) {
          // Create a free day associated with this specific job
          const freeDay: WorkDay = {
            id: `free_${dateString}_${job.id}`,
            date: dateString,
            jobId: job.id, // Associate free day with the specific job
            hours: 0,
            type: 'free',
            overtime: false,
  ...
  static async clearAutoSchedule(jobId: string): Promise<void> {
    ...
    const autoGeneratedDays = workDays.filter(wd => 
      wd.isAutoGenerated && (
        ...
        (wd.jobId === jobId) ||
        // Free days (they are auto-generated and have type 'free' for this specific job)
        (wd.type === 'free' && wd.jobId === jobId)
      )
    );
