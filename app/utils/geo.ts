...
export function checkPreciseLocationNeeded(radius: number, accuracy: number | undefined): {
  ...
  if (radius < 80 && acc > 65) {
    return {
      needsPrecise: true,
      message: `Radio ${radius}m requiere ubicación precisa (actual: ${Math.round(acc)}m). Actívala en Ajustes > Privacidad > Ubicación > Working Time Control > Ubicación precisa.`
    };
  }
