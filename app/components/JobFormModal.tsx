          ...
          <>
            ...
            {formData.salary?.type === 'hourly' && (
              <View ref={overtimeSectionRef} style={styles.financialCard}>
                ...
                <View style={styles.switchRow}>
                  <Text style={styles.label}>{t('job_form.financial.enable_overtime_rate')}</Text>
                  <Switch
                    value={formData.salary?.overtimeEnabled || false}
                    onValueChange={(value) => {
                      if (value && !isSubscribed) {
                        setShowPremiumModal(true);
                      } else {
                        updateNestedData('salary', 'overtimeEnabled', value);
                      }
                    }}
                    trackColor={{ false: colors.separator, true: colors.primary }}
                    thumbColor={formData.salary?.overtimeEnabled ? '#ffffff' : '#f4f3f4'}
                  />
                </View>

                {formData.salary?.overtimeEnabled && (
                  <View style={styles.amountInputContainer}>
                    ...
                    <TextInput
                      style={styles.amountInput}
                      value={formData.salary?.overtimeRate ? String(formData.salary.overtimeRate) : ''}
                      onChangeText={(value) => updateNestedData('salary', 'overtimeRate', Number(value) || 0)}
                      placeholder="22"
                      placeholderTextColor={colors.textTertiary}
                      keyboardType="numeric"
                    />
                ...
                {formData.salary?.overtimeEnabled && (
                  <Text style={styles.amountHelper}>
                    {t('job_form.financial.overtime_rate_helper')}
                  </Text>
                )}
              </View>
            )}
